console.log("////////////////////////////// QUESTION 3.1 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\")

/* 
Given a principal, an interest rate, a compounding frequency, and a time (in years), "computeCompoundInterest" returns the amount of compound interest generated.
Reference:
https://en.wikipedia.org/wiki/Compound_interest#Calculation_of_compound_interest
This shows the formula used to calculate the total compound interest generated.

Starter Code :
*/
function computeCompoundInterest(principal, interestRate, compoundingFrequency, timeInYears) {
  // your code here

  var divide = (interestRate/compoundingFrequency) + 1;
  var square = compoundingFrequency * timeInYears;
  var squared = Math.pow(divide, square);
  var multi = principal * squared;
  var end = multi - principal;
  return end;


    // code ends here
}

var output = computeCompoundInterest(1500, .043, 4, 6);
console.log(output); // --> 438.8368221341061


console.log("////////////////////////////// QUESTION 3.2 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\")

/*Write a function called "keysCounter".

Given an object, "keysCounter" returns how many properties the given object has.

*/
function keysCounter(obj) {
  // your code here

  return Object.keys(obj).length;

  // code ends here
}

var obj = {
  a: 1,
  b: 2,
  c: 3
};

var output = keysCounter(obj);
console.log(output); // ---> 3

console.log("////////////////////////////// QUESTION 3.3 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\")
/*Write a function called "countCharacter".

Given a string input and a character, "countCharacter" returns the number of occurences of a given character in the given string.

For the argument 'I am a hacker' and the letter 'a', the letter 'a' occurs 3 times, so the return value is 3

Starter Code :*/
function countCharacter(str, char) {
  // your code here

  var count = 0;
  for(var i=0; i<str.length; i++)
  {
    if(str[i] === char)
    {
      count++;
    }
  }
  return count




    // code ends here
}
var output = countCharacter('I am a hacker', 'a');
console.log(output); // --> 3
